...
import com.turkcell.atm.IAtm;
import com.turkcell.atm.sql.dao.RegisterDao;
...
public class AtmDto implements IAtm {
	...
	public boolean bankGiris() {
		...
		while (counter > 0) {
			...
			if (counter <= 0) {
				System.out
						.println("Kartınız bloke oldu: Müşteri Hizmetlerini aramamı ister misiniz?\n çıkış yapılıyor");
				System.exit(0);
			}
		}
		return false;
	...
	private void eft() {
		...
		double amount = klavye.nextDouble();
		eftDto.setEftAmount(amount);
	...
	public void paraCek() {
		...
		if (SafeDto.amount <= 0) {
			...
			System.out.println(
					"Cekeceğiniz miktar " + reduceMoney + "ancak Tek seferde 5000 TL kadar  Para çekebilirsiniz");
		} else {
